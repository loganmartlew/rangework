...
- `apps/mobile/shared/src/commonMain/kotlin/com/loganmartlew/rangework/shared/repository/` — repository interfaces: `PracticeUnitRepository`, `PracticeSessionRepository` (thinned to `persist`/`get`/`list`/`delete`), `MeasurementPreferencesRepository`, `ClubRepository`, `TagRepository`.
